...
python src/gen_w_improve.py --config_file run_configs/guppy_full_run_config.yaml --run_name $RUN_NAME
...
python src/test_existing_circuits.py "$ASSEMBLED_DIR" --workers 4 --language guppy
