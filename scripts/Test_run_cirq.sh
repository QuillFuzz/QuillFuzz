...
RUN_NAME="Test_run_cirq_$(date +'%Y%m%d_%H%M%S')"
RUN_DIR="$(pwd)/local_saved_circuits/$RUN_NAME"
python src/gen_w_improve.py --config_file run_configs/cirq_test_run_config.yaml --run_name "$RUN_NAME" --output_dir "$RUN_DIR"
