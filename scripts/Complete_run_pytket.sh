...
RUN_NAME="Complete_run_pytket_$(date +'%Y%m%d_%H%M%S')"
RUN_DIR="$(pwd)/local_saved_circuits/$RUN_NAME"
python src/gen_w_improve.py --config_file run_configs/pytket_full_run_config.yaml --run_name "$RUN_NAME" --output_dir "$RUN_DIR"
