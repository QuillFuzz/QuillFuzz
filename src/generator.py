...
def process_single_program(index, model, generated_dir, failed_dir, n_max_fixing_cycles, logfile, log_lock, start_time, language, prompt_dir, verbose=False):
    ...
    current_stats = {'cost': 0.0, 'prompt_tokens': 0, 'completion_tokens': 0, 'total_tokens': 0, 'quality_score': None, 'metrics': {'compilation': {}, 'execution': {}}}
    ...
    if not os.path.exists(generation_prompt_path):
        write_log(f"Generation prompt not found at {generation_prompt_path}\n")
    ...
    if generated_program is None:
        write_log(f"Failed to generate output for {filename}. Error: {gen_error}\n\n")
        return None, current_stats
    ...
    if stats:
            ...
            write_log(f"{filename} Generation Cost: ${stats.get('cost', 0.0):.6f} | Input Tokens: {stats.get('prompt_tokens', 0)} | Output Tokens: {stats.get('completion_tokens', 0)} | Total Tokens: {stats.get('total_tokens', 0)}\n")
    # ------------------------------------------------------------------------- #

    # Compile check
    # ------------------------------------------------------------------------- #
    write_log(f"--- {get_elapsed()} Testing generated {filename} ---\n")
    
    compile_error, _, compile_metrics, wrapped_compile_code = compile_generated_program(generated_program, language=language)

    current_stats['metrics']['compilation'] = compile_metrics

    if compile_error:
        run_error = compile_error
        current_stats['quality_score'] = compile_metrics.get('quality_score', 0.0)
        write_log(f"{filename} Compilation Failed:\n{compile_error}\n\n")
        if verbose:
            write_log(f"--- {filename} Code ---\n{wrapped_compile_code}\n-----------------------\n\n")
    else:
        run_error = ""

    if run_error == "":
        # Compiles successfully, now run it
        write_log(f"--- {get_elapsed()} Running {filename} (Compilation passed) ---\n")
            
        run_error, _, metrics, runtime_code = run_generated_program(generated_program, language=language)
        
        current_stats['metrics']['execution'] = metrics
        current_stats['quality_score'] = metrics.get('quality_score', 0.0)

        write_log(f"{filename} Metrics: {current_stats['metrics']}\n")

        if run_error.strip() == "":
             write_log(f"{filename} ran successfully with no errors.\n\n")
             save_path = os.path.join(generated_dir, filename)
             save_text_to_file(generated_program, save_path)
             return save_path, current_stats
        else:
             write_log(f"{filename} Runtime Error (Skipping fix as it compiled):\n{run_error}\n\n")
             if verbose:
                  # Logging wrapped code to see what was actually executed
                  write_log(f"--- {filename} Code ---\n{runtime_code}\n-----------------------\n\n")
        ...
        for cycle in range(n_max_fixing_cycles):
            ...
            if not os.path.exists(fixing_prompt_path):
                write_log(f"Fixing prompt not found at {fixing_prompt_path}, skipping fix.\n")
                break
            ...
            if fixed_code is None:
                write_log(f"Failed to generate fix for {filename} cycle {cycle+1}. Error: {fix_error}\n\n")
                break
            ...
            if stats:
                ...
                current_stats['completion_tokens'] += stats.get('completion_tokens', 0)
                current_stats['total_tokens'] += stats.get('total_tokens', 0)
                write_log(f"{filename} Fixing (Cycle {cycle+1}) Cost: ${stats.get('cost', 0.0):.6f} | Input Tokens: {stats.get('prompt_tokens', 0)} | Output Tokens: {stats.get('completion_tokens', 0)} | Total Tokens: {stats.get('total_tokens', 0)}\n")

            write_log(f"--- {get_elapsed()} Verifying fixed {filename} (Cycle {cycle+1}) ---\n")
            
            # Check compilation only
            fixed_compile_error, _, fixed_compile_metrics, _ = compile_generated_program(fixed_code, language=language)
            
            current_stats['metrics']['compilation'] = fixed_compile_metrics

            if fixed_compile_error:
                current_stats['quality_score'] = fixed_compile_metrics.get('quality_score', 0.0)
                write_log(f"Fixed {filename} (Cycle {cycle+1}) Compilation Failed:\n{fixed_compile_error}\n\n")
                if verbose:
                     write_log(f"--- Fixed {filename} (Cycle {cycle+1}) Code ---\n{fixed_code}\n-----------------------------------\n\n")
                current_code = fixed_code
                current_error = fixed_compile_error
            else:
                # Compiled successfully!
                write_log(f"Fixed {filename} (Cycle {cycle+1}) compiled successfully.\n")
                
                # Now run it
                write_log(f"--- {get_elapsed()} Running fixed {filename} (Cycle {cycle+1}) ---\n")
                ...
                current_stats['metrics']['execution'] = fixed_metrics

                write_log(f"{filename} Cycle {cycle+1} Metrics: {current_stats['metrics']}\n")

                if fixed_run_error.strip() == "":
                    write_log(f"Fixed {filename} (Cycle {cycle+1}) ran successfully with no errors.\n\n")
                    save_path = os.path.join(generated_dir, filename)
                    save_text_to_file(fixed_code, save_path)
                    return save_path, current_stats
                else:
                    write_log(f"Fixed {filename} (Cycle {cycle+1}) compiled but failed Runtime:\n{fixed_run_error}\n\n")
                    if verbose:
                         write_log(f"--- Fixed {filename} (Cycle {cycle+1}) Code ---\n{fixed_code}\n-----------------------------------\n\n")
        ...
        if not fixed:
            write_log(f"Failed to fix {filename}. Saving to failed_programs.\n\n")
...
def main():
    ...
    for model in args.models:
        ...
        with open(logfile_path, "a", buffering=1) as logfile:
            ...
            coverage_target = "guppylang" if args.language == "guppy" else "qiskit"
            logfile.write(f"Coverage Target: {coverage_target}\n")
            logfile.write(f"Started at: {time.ctime(start_time)}\n\n")

            if args.verbose:
                gen_prompt_path = os.path.join(args.prompt_dir, "generation_prompt.txt")
                fix_prompt_path = os.path.join(args.prompt_dir, "fixing_prompt_template.txt")
                
                if os.path.exists(gen_prompt_path):
                    with open(gen_prompt_path, 'r') as f:
                        logfile.write(f"--- Generation Prompt Template ---\n{f.read()}\n----------------------------------\n\n")
                
                if os.path.exists(fix_prompt_path):
                    with open(fix_prompt_path, 'r') as f:
                        logfile.write(f"--- Fixing Prompt Template ---\n{f.read()}\n------------------------------\n\n")
